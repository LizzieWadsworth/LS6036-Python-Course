...
Below I've used a 500 bp window to illustrate what this can look like - as we only have a small section of the actual dataset! 

I've plotted the number of variants against the middle point (between start and stop position) for each window.  
...
for i in range(len(counts)):
    ...
    start_position = locations[i][0]
    stop_position = locations[i][1]
    midpoint = start_position + (window_size/2)
    midpoints.append(midpoint)
    variant_list.append(variants)
    #print(f"{variants} varients found between {start_position} and {stop_position}")

plt.plot(midpoints, variant_list)
plt.xlim(0,max(midpoints)+(window_size/2))
plt.xlabel("Position on chromosome")
plt.ylabel("Number of variants")
plt.title("Number of variants per 500 bp window on chromosome 2L")
plt.show()
