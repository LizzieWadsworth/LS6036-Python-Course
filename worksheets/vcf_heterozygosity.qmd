...
Can you plot the percent (%) heterozygositity for both samples on a bar chart? 
...
# Calculate percentage heterozygosity by dividing heterozygous alleles by total alleles and multiply by 100
sample1_heterozygosity = (het_count / total_sites) * 100

chosen_sample = 1 # Second sample is index 1
sample_genotype = all_genotypes[:, chosen_sample] # Extract sample result
...
sample2_heterozygosity = (het_count / total_sites) * 100

plt.bar([samples[0], samples[1]], [sample1_heterozygosity, sample2_heterozygosity], color = ['orchid', 'seagreen'])
plt.ylim(0,100)
plt.ylabel("Percent heterozygosity")
plt.xlabel("Sample")
plt.title("Heterozygosity of samples")
plt.show()
